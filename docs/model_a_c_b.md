...
C is not an error. C is mediation that transforms A into A′, whether it arises from model-side generation or execution structure.
...
This shifts governance from output correction to workflow-level control of transformation conditions.
...
In its shortest form, drift is the failure to preserve A's intended identity.|A′    |Internally reconstructed state: A as transformed by C|
|B     |Intended output (what the user expects)|
|B′    |Actual output (what the system produces)|
